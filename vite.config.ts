...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      // Forward /api/* to the local AI proxy (server/server.mjs) that holds
      // the AI provider key — the key never reaches the browser bundle.
      proxy: {
        '/api': {
          target: process.env.AI_PROXY_TARGET || 'http://localhost:8788',
          changeOrigin: true,
        },
      },
      watch: { ignored: ['**/.figma/**'] },
